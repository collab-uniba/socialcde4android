...
public class SetAvatar_Operation implements Operation {
	...
	public Bundle execute(Context context, Request request)
		...
		String uri = request.getString(Consts.URI);
		int status =0;
		String result = "";
		...
		try {
			...
			if (status >= 200 && status <= 299) {
				...
				throw new ConnectionException("Error ",Error_consts.SET_AVATAR_ERROR);
			...
			throw new ConnectionException("Error ",Error_consts.SET_AVATAR_ERROR * Error_consts.TIMEOUT_FACTOR);
		}  catch (Exception e) {
			throw new ConnectionException("Error ",Error_consts.SET_AVATAR_ERROR);
		}
		...
		if (result.equals("true")) {
			bundle.putBoolean(Consts.SETTED_AVATAR, true);
			bundle.putString(Consts.URI, uri);

		} else {
			bundle.putBoolean(Consts.SETTED_AVATAR, false);

		}
		bundle.putInt(Consts.REQUEST_TYPE, Consts.REQUESTTYPE_SET_AVATAR);
		return bundle;
